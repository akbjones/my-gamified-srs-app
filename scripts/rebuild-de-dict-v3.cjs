 ...
 *  8. Apply to de.ts, preserve IPA/lemma/pos + CONTRACTION_MAP + IRREGULAR_MAP + lookupWord
 *  9. 100-entry random review -> scripts/output/de-v3-review.md
 *
 * NOTE: German dict uses `const DICT` not `export const dictionary`.
 */
...
const FUNCTION_WORDS = {
  ...
  'unser': 'our', 'unsere': 'our (f./pl.)',
  'euer': 'your (pl.)',

  // ── Interrogative pronouns ──
  'wer': 'who', 'was': 'what', 'wo': 'where', 'wann': 'when',
  'wie': 'how', 'warum': 'why',
  'welch': 'which', 'welche': 'which (f./pl.)', 'welchen': 'which (acc.)', 'welchem': 'which (dat.)',
...
function collectDeckWords() {
  ...
  for (const card of deck) {
    ...
    for (const tok of tokens) {
      ...
      // Only keep Latin-script tokens (German uses Latin + umlauts)
...
// German dict uses `const DICT: Record<string, DictEntry> = {`
// ──────────────────────────────────────────────────────────────
function parseExistingDict() {
  ...
  // Find the closing `};` of the DICT block (the first `};` after dictStartIdx)
  let footer = '';
  if (dictStartIdx >= 0) {
    // We need to find the matching `};` for DICT
    // Simple approach: find the line that is just `};` after the dict entries
    const afterDict = src.slice(dictStartIdx);
    // The DICT block ends with a line that has just `};`
...
// ──────────────────────────────────────────────────────────────
async function main() {
  console.log('=== German Dictionary Rebuild v3 ===\n');
  ...
  for (const word of dedupedWords) {
    ...
    if (en.length > 0 && en[0] === en[0].toUpperCase() && en[0] !== en[0].toLowerCase()) {
      ...
      const isProperNoun = /^(Germany|Berlin|Munich|Hamburg|Frankfurt|Cologne|Christmas|Easter|Europe|Austria|Switzerland)$/i.test(en);
  ...
  let md = `# German Dictionary v3 Rebuild - 100-Entry Review\n\n`;
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'de-v3-review.md'), md, 'utf8');
  console.log(`\nReview written to scripts/output/de-v3-review.md`);
