#!/usr/bin/env node
/**
 * Rebuild Swedish dictionary v3 – uses shared pipeline utilities.
 *
 * Steps:
 *  1. Swedish function word table (~130 entries, never sent to Google)
 ...
 *  9. 100-entry random review -> scripts/output/sv-v3-review.md
 *
 * NOTE: Swedish dict uses `const dictionary: Record<string, DictEntry>`.
 */
...
// ──────────────────────────────────────────────────────────────
// Step 1: Swedish function word table (~130 entries)
// ──────────────────────────────────────────────────────────────
...
function collectDeckWords() {
  ...
  for (const card of deck) {
    ...
    for (const tok of tokens) {
      ...
      // Only keep Latin-script tokens (Swedish uses Latin + åäö)
...
// Swedish dict uses `const dictionary: Record<string, DictEntry> = {`
// ──────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────
async function main() {
  console.log('=== Swedish Dictionary Rebuild v3 ===\n');
  ...
  let md = `# Swedish Dictionary v3 Rebuild - 100-Entry Review\n\n`;
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'sv-v3-review.md'), md, 'utf8');
  console.log(`\nReview written to scripts/output/sv-v3-review.md`);
