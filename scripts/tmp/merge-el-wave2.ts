/**
 * Merge Greek wave 2 (slices A–D, el-0301..el-1300) into deck + dict +
 * KNOWN_VERBS, with the wave-5 Indonesian gates.
 *
 *   npx tsx scripts/tmp/merge-el-wave2.ts          # check only
 *   npx tsx scripts/tmp/merge-el-wave2.ts --write  # write deck + dict + verbs
 */
...
for (const b of ['A', 'B', 'C', 'D']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave2-el-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave2-el-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) {
    const key = normalizeGreek(k);
    if (!dict[key]) dict[key] = v;
  }
  try {
    for (const r of JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave2-el-verbs-${b}.json`, 'utf8'))) newVerbs.add(r);
...
newCards.forEach((c, i) => {
  const want = `el-${String(i + 301).padStart(4, '0')}`;
...
if (write) {
  ...
  if (fresh.length) {
    const anchor = "  'ονομάζομαι', 'σκέφτομαι', 'χρειάζομαι', 'εργάζομαι', 'ετοιμάζομαι',\n];";
    if (!eng.includes(anchor)) { console.log('verbs anchor missing'); process.exit(1); }
    eng = eng.replace(anchor, `  'ονομάζομαι', 'σκέφτομαι', 'χρειάζομαι', 'εργάζομαι', 'ετοιμάζομαι',\n  // wave-2 verbs\n  ${fresh.map(v => `'${v}'`).join(', ')},\n];`);
