/**
 * Merge Greek wave 3 (slices A–D, el-1301..el-2300) into deck + dict +
 * KNOWN_VERBS, with the wave-5 Indonesian gates.
 *
 *   npx tsx scripts/tmp/merge-el-wave3.ts          # check only
 *   npx tsx scripts/tmp/merge-el-wave3.ts --write  # write deck + dict + verbs
 */
...
for (const b of ['A', 'B', 'C', 'D']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-el-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-el-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) {
    const key = normalizeGreek(k);
    if (!dict[key]) dict[key] = v;
  }
  try {
    for (const r of JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-el-verbs-${b}.json`, 'utf8'))) newVerbs.add(r);
...
newCards.forEach((c, i) => {
  const want = `el-${String(i + 1301).padStart(4, '0')}`;
