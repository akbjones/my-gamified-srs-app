/**
 * Merge growth-batch 2 (slices A–E, id-0301..id-0800) into the existing
 * Indonesian deck + dictionary, with the same gates as the pilot merge.
 *
 *   npx tsx scripts/tmp/merge-id-batch2.ts          # check only
 *   npx tsx scripts/tmp/merge-id-batch2.ts --write  # write deck + dict + roots
 */
...
for (const b of ['A', 'B', 'C', 'D', 'E']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/pilot-id2-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/pilot-id2-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) {
    const key = k.toLowerCase();
    if (!dict[key]) dict[key] = v;
  }
  try {
    for (const r of JSON.parse(readFileSync(`${ROOT}scripts/tmp/pilot-id2-roots-${b}.json`, 'utf8'))) newRoots.add(r);
...
newCards.forEach((c, i) => {
  const want = `id-${String(i + 301).padStart(4, '0')}`;
...
if (write) {
  ...
  if (newRoots.size) {
    ...
    const anchor = "  'ada', 'boleh', 'belok', 'jumpa', 'lewat', 'terbit', 'minta', 'turun',";
    const addition = `${anchor}\n  // batch-2 roots\n  ${[...newRoots].sort().map(r => `'${r}'`).join(', ')},`;
    if (!eng.includes('// batch-2 roots')) eng = eng.replace(anchor, addition);
