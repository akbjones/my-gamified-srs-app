/**
 * Merge wave 3 (slices A–H, id-1801..id-2800) into the existing
...
for (const b of ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave4-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave4-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) {
    const key = k.toLowerCase();
    if (!dict[key]) dict[key] = v;
  }
  try {
    for (const r of JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave4-roots-${b}.json`, 'utf8'))) newRoots.add(r);
...
newCards.forEach((c, i) => {
  const want = `id-${String(i + 1801).padStart(4, '0')}`;
...
if (write) {
  ...
  if (newRoots.size) {
    ...
    const addition = `  // wave-4 roots\n  ${[...newRoots].sort().map(r => `'${r}'`).join(', ')},\n${anchor}`;
    if (!eng.includes('// wave-4 roots')) eng = eng.replace(anchor, addition);
