/**
 * Merge wave 3 (slices A–H, id-2801..id-3933) into the existing
 * Indonesian deck + dictionary, with the same gates as the pilot merge.
 *
 *   npx tsx scripts/tmp/merge-id-wave4.ts          # check only
 *   npx tsx scripts/tmp/merge-id-wave4.ts --write  # write deck + dict + roots
 */
...
for (const b of ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave5-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave5-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) {
    const key = k.toLowerCase();
    if (!dict[key]) dict[key] = v;
  }
  try {
    for (const r of JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave5-roots-${b}.json`, 'utf8'))) newRoots.add(r);
...
newCards.forEach((c, i) => {
  const want = `id-${String(i + 2801).padStart(4, '0')}`;
...
for (const c of newCards) {
  // node-36 (id-3176..3300) is the policy-rule-3 sanctioned colloquial slice — every
  // colloquial there is deliberately modeled WITH a register tip; skip the scan.
  const n = parseInt(c.id.slice(3), 10);
  if (n >= 3176 && n <= 3300) continue;
...
if (write) {
  ...
  if (newRoots.size) {
    ...
    const addition = `  // wave-5 roots\n  ${[...newRoots].sort().map(r => `'${r}'`).join(', ')},\n${anchor}`;
    if (!eng.includes('// wave-5 roots')) eng = eng.replace(anchor, addition);
