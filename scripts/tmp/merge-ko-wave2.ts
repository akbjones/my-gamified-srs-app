/**
 * Merge Korean wave 2 (slices A–D, ko-0301..ko-1300) into deck + dict +
 * engine (KNOWN_VERBS + IRREGULARS), with the standard wave gates.
 *
 *   npx tsx scripts/tmp/merge-ko-wave2.ts          # check only
 *   npx tsx scripts/tmp/merge-ko-wave2.ts --write  # write deck + dict + verbs
 */
...
for (const b of ['A', 'B', 'C', 'D']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave2-ko-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave2-ko-dict-${b}.json`, 'utf8'));
...
newCards.forEach((c, i) => {
  const want = `ko-${String(i + 301).padStart(4, '0')}`;
...
if (write) {
  ...
  const kvAnchor = "  '사랑하다', '전화하다', '운동하다', '요리하다', '시작하다', '도착하다',\n];";
  if (!eng.includes(kvAnchor)) { console.log('KNOWN_VERBS anchor missing'); process.exit(1); }
  if (freshReg.length) {
    eng = eng.replace(kvAnchor, `  '사랑하다', '전화하다', '운동하다', '요리하다', '시작하다', '도착하다',\n  // wave-2 verbs\n  ${freshReg.map(v => `'${v}'`).join(', ')},\n];`);
  }
  const irrAnchor = "  '배우다': '배워요',\n};";
  if (!eng.includes(irrAnchor)) { console.log('IRREGULARS anchor missing'); process.exit(1); }
  const freshIrr = [...irregulars].filter(([d]) => !eng.includes(`'${d}':`));
  if (freshIrr.length) {
    eng = eng.replace(irrAnchor, `  '배우다': '배워요',\n  // wave-2 irregulars\n${freshIrr.map(([d, h]) => `  '${d}': '${h}',`).join('\n')}\n};`);
  }
