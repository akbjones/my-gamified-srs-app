/**
 * Merge Korean wave 3 (slices A–D, ko-1301..ko-2300) into deck + dict +
 * engine (KNOWN_VERBS + IRREGULARS), with the standard wave gates.
 *
 *   npx tsx scripts/tmp/merge-ko-wave3.ts          # check only
 *   npx tsx scripts/tmp/merge-ko-wave3.ts --write  # write deck + dict + verbs
 */
...
for (const b of ['A', 'B', 'C', 'D']) {
  newCards.push(...JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-ko-cards-${b}.json`, 'utf8')));
  const d: Record<string, DictEntry> = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-ko-dict-${b}.json`, 'utf8'));
  for (const [k, v] of Object.entries(d)) if (!dict[k]) dict[k] = v;
  try {
    const vs: VerbSpec[] = JSON.parse(readFileSync(`${ROOT}scripts/tmp/wave3-ko-verbs-${b}.json`, 'utf8'));
    for (const v of vs) {
...
newCards.forEach((c, i) => {
  const want = `ko-${String(i + 1301).padStart(4, '0')}`;
