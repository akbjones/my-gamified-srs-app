/**
 * Validation for slice C (ko-0151..ko-0225, node-03) — uses the REAL engine.
 * Run: npx tsx scripts/tmp/validate-koC.ts
 */
...
const cards: any[] = JSON.parse(readFileSync(join(dir, 'ko-cards-C.json'), 'utf8'));
const myDict: Record<string, { en: string; ipa: string; pos?: string; lemma?: string }> =
  JSON.parse(readFileSync(join(dir, 'ko-dict-C.json'), 'utf8'));
const verbsOut: (string | [string, string])[] = JSON.parse(readFileSync(join(dir, 'ko-verbs-C.json'), 'utf8'));
...
cards.forEach((c, i) => {
  ...
  if (c.grammarNode !== 'node-03') errors.push(`${c.id}: bad grammarNode ${c.grammarNode}`);
  ...
  if (wc < 2 || wc > 8) errors.push(`${c.id}: word count ${wc} out of Q1 band 2-8: ${c.target}`);
  if (c.grammar) {
...
for (const c of cards) {
  ...
  const toks = t.split(/\s+/).map((x: string) => x.replace(/[^가-힣]/g, ''));
  for (const tok of toks) if (OFFENDER_TOKEN.includes(tok)) errors.push(`${c.id}: offender token ${tok}`);
  const last = toks[toks.length - 1];
  if (!last.endsWith('요')) errors.push(`${c.id}: sentence does not end in 해요체 -요: ${t}`);
}
...
for (const [k, v] of Object.entries(myDict)) {
  ...
  if (expected !== k) errors.push(`dict-C: ${k} vs derived ${expected} (${v.lemma})`);
}

// ── 5. Report ─────────────────────────────────────────────────────
const tips = cards.filter(c => c.grammar).length;
const extraTagged = cards.filter(c => c.tags.length > 1).length;
const tagCounts: Record<string, number> = {};
cards.forEach(c => c.tags.forEach((t: string) => (tagCounts[t] = (tagCounts[t] ?? 0) + 1)));

console.log('cards:', cards.length, `(ko-0151..ko-0225)`);
...
console.log('tags:', JSON.stringify(tagCounts), `| cards with extra tag: ${extraTagged} (${((extraTagged / cards.length) * 100).toFixed(0)}%)`);
