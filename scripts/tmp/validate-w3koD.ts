/**
 * Validation for wave3 slice D (ko-2051..ko-2300, node-19/node-20) — REAL engine.
 * Run: npx tsx scripts/tmp/validate-w3koD.ts
 * Coverage-only listing: npx tsx scripts/tmp/validate-w3koD.ts --uncovered
 */
...
const cards: any[] = JSON.parse(readFileSync(join(dir, 'wave3-ko-cards-D.json'), 'utf8'));
const dictPath = join(dir, 'wave3-ko-dict-D.json');
...
const verbsPath = join(dir, 'wave3-ko-verbs-D.json');
const verbsOut: (string | [string, string])[] =
  existsSync(verbsPath) ? JSON.parse(readFileSync(verbsPath, 'utf8')) : [];
...
cards.forEach((c, i) => {
  ...
  const wantNode = n <= 2175 ? 'node-19' : 'node-20';
  if (c.grammarNode !== wantNode) errors.push(`${c.id}: bad grammarNode ${c.grammarNode} (want ${wantNode})`);
...
for (const lemma of [...newRegular, ...Object.keys(irrMap)]) {
  ...
  // connectives: stem+고 (post-merge the reverse map indexes these too)
  const stemGo = lemma.slice(0, -1) + '고';
  if (!verbFormLemma.has(stemGo)) verbFormLemma.set(stemGo, lemma);
}
...
// (먹고, 놓고) and the -아/어서 connective (많아서, 세서).
...
for (const c of cards) {
  for (const raw of c.target.split(/\s+/)) {
    ...
    // Latin/digit-prefixed tokens (KTX가, 2박): try raw dict key first
    const rawKey = raw.replace(/[.,?!]/g, '');
    const w = myDict[rawKey] || lookupWord(rawKey) ? rawKey : hangul;
    if (!w) continue;
    tokenCount++;
    const entry = myDict[w] ?? lookupWord(w) ?? resolve(w);
...
if (!uncoveredOnly) {
  ...
  // register scan: every sentence in every card ends in 해요체 -요
  for (const c of cards) {
    for (const sent of c.target.split(/(?<=[.?!])\s+/)) {
      const bare = sent.replace(/[.,?!]/g, '').trim();
      if (!bare.endsWith('요')) errors.push(`${c.id}: sentence not 해요체: ${sent}`);
    }
  }
}
...
console.log('cards:', cards.length, '(ko-2051..ko-2300)');
