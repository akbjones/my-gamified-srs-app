/**
 * Validation for wave2 slice B (ko-0551..ko-0800, node-07/node-08).
 * Uses the REAL engine. Run: npx tsx scripts/tmp/validate-wave2-koB.ts
 */
...
const cards: any[] = JSON.parse(readFileSync(join(dir, 'wave2-ko-cards-B.json'), 'utf8'));
const myDict: Record<string, { en: string; ipa: string; pos?: string; lemma?: string }> =
  JSON.parse(readFileSync(join(dir, 'wave2-ko-dict-B.json'), 'utf8'));
const verbsOut: (string | { dict: string; haeyo: string })[] =
  JSON.parse(readFileSync(join(dir, 'wave2-ko-verbs-B.json'), 'utf8'));

const errors: string[] = [];

// ── 1. Card structure ─────────────────────────────────────────────
if (cards.length !== 250) errors.push(`expected 250 cards, got ${cards.length}`);
const existingTargets = new Set((deck as any[]).map(c => c.target));
const seenTargets = new Set<string>();
cards.forEach((c, i) => {
  const n = 551 + i;
  const id = `ko-${String(n).padStart(4, '0')}`;
  const node = n <= 675 ? 'node-07' : 'node-08';
  ...
  if (existingTargets.has(c.target)) errors.push(`${c.id}: duplicates EXISTING deck sentence: ${c.target}`);
...
for (const v of [...newRegular, ...Object.keys(irrMap)]) {
  if (KNOWN_VERBS.includes(v)) errors.push(`verbs-B: ${v} already in KNOWN_VERBS`);
  if (!v.endsWith('다')) errors.push(`verbs-B: ${v} not a -다 form`);
}
for (const v of newRegular) {
  if (!haeyo(v)) errors.push(`verbs-B: engine cannot derive ${v}`);
}
...
function expectedForms(lemma: string): Set<string> {
  ...
  // future is only trusted from the engine for non-ㅂ/르-seeded irregulars;
  // for seeded irregular classes we simply don't allow future usage.
  if (!irrMap[lemma]) {
...
// findInfinitive('다') crashes (empty stem decompose) — guard it.
...
for (const c of cards) {
  ...
  for (let ti = 0; ti < rawTokens.length; ti++) {
    ...
    // Dict-entry precedence mirrors the real lookupWord: an explicit dict
    // entry (e.g. counter 줄) wins over the conjugation reverse map (줄 거예요).
...
for (const [k, v] of Object.entries(myDict)) {
  if (!v.en || !v.ipa) errors.push(`dict-B: ${k} missing en/ipa`);
  if (!v.lemma || v.lemma === '이다') continue;
  if (!get(v.lemma)) errors.push(`dict-B: lemma ${v.lemma} of ${k} has no entry`);
  const exp = expectedForms(v.lemma);
  if (!exp.has(k) && !exp.has(`${k} 거예요`))
    errors.push(`dict-B: ${k} is not a present/past/future form of ${v.lemma} (${[...exp].join(', ')})`);
}

// register offenders scan
const offenders = ['당신', '금일', '명일', '본인', '귀하', '하오니', '바랍니다'];
for (const c of cards) {
  for (const o of offenders) {
    if (c.target.includes(o)) errors.push(`${c.id}: register offender ${o}`);
  }
...
console.log('cards:', cards.length, '(ko-0551..ko-0800)');
console.log('unique sentences:', seenTargets.size, '| clash with existing deck: checked');
