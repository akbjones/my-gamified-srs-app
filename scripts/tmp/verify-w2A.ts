/** Verify wave-2 slice A (el-0301..el-0550) against the brief. */
...
const cards: any[] = JSON.parse(readFileSync(`${ROOT}/scripts/tmp/wave2-el-cards-A.json`, 'utf8'));
const dict: Record<string, any> = JSON.parse(readFileSync(`${ROOT}/scripts/tmp/wave2-el-dict-A.json`, 'utf8'));
const verbs: string[] = JSON.parse(readFileSync(`${ROOT}/scripts/tmp/wave2-el-verbs-A.json`, 'utf8'));
...
// 1. structure: ids el-0301..0550, priority, audio, node ranges, tags
...
// 2. unique sentences (within slice AND vs existing deck)
const keyOf = (t: string) =>
  normalizeGreek(t).replace(/[^α-ωάέήίόύώϊϋΐΰ\s]/g, '').replace(/\s+/g, ' ').trim();
const seen = new Map<string, string>();
for (const c of existingDeck) seen.set(keyOf(c.target), c.id);
...
// 7. every verb form used in a card belongs to a checked dict entry with accents intact
//    (tips must show accented forms — sample scan: every Greek word in tips has an accent
//    unless monosyllabic)
for (const c of cards) {
  if (!c.grammar) continue;
  for (const w of c.grammar.split(/[\s,:;()=!;·—]+/)) {
    const g = w.replace(/[^α-ωάέήίόύώΆ-Ώα-ω]/gi, '');
    if (!/[α-ωΑ-Ω]/i.test(g)) continue;
    const bare = normalizeGreek(g);
    const nvow = (bare.match(/[αεηιουωάέήίόύώ]/g) ?? []).length;
    if (nvow >= 2 && stripAccents(bare) === bare && !/^(θα|να|δεν|μη|και|του|τησ|την|τον|το|τα|τη|με|σε|για|απο|ποτε|μου|σου|μασ|σασ|τουσ|τισ|οτι|που|ενω)$/.test(bare)) {
...
console.log(`new verbs: ${verbs.join(', ')}`);
