...
const GamificationHub: React.FC<GamificationHubProps> = ({ stats, achievements, retention, onBack }) => {
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 animate-fade-in pb-12 pt-10">
      <button onClick={onBack} className="btn-ghost text-[10px]">← Back</button>

      {/* Profile Header */}
      <div className="stat-card p-6 border-indigo-500/20 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-500/5 blur-3xl rounded-full -mr-12 -mt-12 pointer-events-none" />
        <div className="relative z-10 flex items-center gap-5">
          <div className="w-16 h-16 bg-slate-900 rounded-2xl border-2 border-indigo-500/40 flex items-center justify-center text-2xl shrink-0">
            🧙‍♂️
          </div>
          <div className="flex-1 min-w-0 space-y-2">
            <h1 className="text-xl font-black">Level {stats.level}</h1>
            <p className="text-indigo-400 text-sm font-bold">{stats.xp} XP</p>
            <div>
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="stat-card text-center py-3">
          <div className="text-xl font-black text-amber-400">{stats.streak}</div>
          <div className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Streak</div>
        </div>
        <div className="stat-card text-center py-3">
          <div className="text-xl font-black text-blue-400">{stats.totalReviews}</div>
          <div className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Reviews</div>
        </div>
        <div className="stat-card text-center py-3">
          <div className="text-xl font-black text-slate-300">{stats.cardsLearned}</div>
          <div className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Learned</div>
        </div>
        <div className="stat-card text-center py-3">
          <div className="text-xl font-black text-emerald-400">{retention}%</div>
          <div className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Retention</div>
        </div>
      </div>
